...
class ImportLolJsons:
    ...
    urls = {
        ...
        "regions": lolDDragon +
        "/realms/euw.json",
        "languages": lolDDragon +
        "/cdn/languages.json",
        "champions": lolDDragon +
        "/cdn/{version}/data/en_US/champion.json",
        "summonerSpells": lolDDragon +
        "/cdn/{version}/data/en_US/summoner.json",
        "profileIcons": lolDDragon +
        "/cdn/{version}/data/en_US/profileicon.json",
        "items": lolDDragon +
        "/cdn/{version}/data/en_US/item.json",
        "summoner": lolApiEuw +
        "/lol/summoner/v4/summoners/by-name/{name}",
        "matches": lolApiEurope +
        "/lol/match/v5/matches/by-puuid/{puuid}/ids",
        "match": lolApiEurope +
        "/lol/match/v5/matches/{matchId}",
        "matchTimeline": lolApiEurope +
        "/lol/match/v5/matches/{matchId}/timeline",
        "featuredMatches": lolApiEuw +
        "/lol/spectator/v4/featured-games",
        "spectator": lolApiEuw +
        "/lol/spectator/v4/active-games/by-summoner/{encryptedSummonerId}",
        "challenges": lolApiEuw +
        "/lol/challenges/v1/player-data/{puuid}",
        "championsMasteries": lolApiEuw +
    ...
    def doGet(self, url: str) -> Optional[Dict[str, Any]]:
        try:
            response = self.session.get(
                url, params={"api_key": self.RIOT_API_KEY})
    ...
    def import_jsons(self):
        ...
        if self.RIOT_API_KEY is None:
            ...
            try:
                ...
                self.datas["champions"] = self.doGet(
                    self.urls["champions"].format(
                        version=self.datas["versions"][0]))
                self.datas["summonerSpells"] = self.doGet(
                    self.urls["summonerSpells"].format(
                        version=self.datas["versions"][0]))
                self.datas["profileIcons"] = self.doGet(
                    self.urls["profileIcons"].format(
                        version=self.datas["versions"][0]))
                self.datas["items"] = self.doGet(
                    self.urls["items"].format(
                        version=self.datas["versions"][0]))
                self.datas["featuredMatches"] = self.doGet(
                    self.urls["featuredMatches"])
                summonerName = self.datas['featuredMatches']['gameList'][0]['participants'][0]['summonerName']

                self.datas["summoner"] = self.doGet(
                    self.urls["summoner"].format(name=summonerName))
                self.datas['spectator'] = self.doGet(
                    self.urls['spectator'].format(
                        encryptedSummonerId=self.datas['summoner']['id']))
                self.datas["matches"] = self.doGet(
                    self.urls["matches"].format(
                        puuid=self.datas["summoner"]["puuid"]))
                matchId = self.datas["matches"][0]
                self.datas["match"] = self.doGet(
                    self.urls["match"].format(matchId=matchId))
                self.datas["matchTimeline"] = self.doGet(
                    self.urls["matchTimeline"].format(matchId=matchId))
                self.datas["challenges"] = self.doGet(
                    self.urls["challenges"].format(
                        puuid=self.datas["summoner"]["puuid"]))
                self.datas["championsMasteries"] = self.doGet(
